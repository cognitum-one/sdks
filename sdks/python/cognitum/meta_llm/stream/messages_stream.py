...
Messages streaming on top of the same generic :mod:`cognitum.sse` parser
``chat_completions_stream`` (PR #88) wired onto. This module is
intentionally the exact same shape as ``.chat_completions_stream`` -- only
the URL path, the event-decode call, and the terminal condition differ;
see that module's docstring for the full retry/timeout/cancellation
rationale, which applies here unchanged.

Pre-byte behavior mirrors ``nonstream.post_json_idempotent`` (credential
acquisition, 401-refresh-once, bounded 429/502/503 retry) with the same
deliberate difference as chat-completions streaming: ADR-0024a §D7
generates an SDK idempotency key only for "a direct nonstream call" --
streams are excluded -- so no ``Idempotency-Key`` header is sent here
either.

Post-byte behavior is the ADR-0023 §D6 / ADR-0024a §D5 contract: once any
response byte has been read, there is NO retry, period.

The wire terminal condition is ``message_stop`` (ADR-0024a §D5 ground
truth: "there is no ``[DONE]`` sentinel like OpenAI; ``message_stop`` is
the wire terminal condition") -- NOT ``[DONE]``/``finish_reason``, which
are OpenAI-specific.
"""
...
if TYPE_CHECKING:
    ...
    from cognitum.meta_llm.types import AnthropicMessageRequest

_PRODUCT = "meta-llm"
_OPERATION = "messages.create_stream"
...
) -> list[MetaLlmStreamEnvelope[AnthropicStreamEvent]]:
    decoded = decode_anthropic_sse_event(raw_event)
    envelopes = []
...
async def _read_sse_body(
    ...
    try:
        while True:
            ...
            # socket cannot hang this generator forever.
            ...
            for raw_event in raw_events:
                for envelope in _build_envelopes(raw_event, request_id, sequence_box):
                    if envelope.event.type == "message_stop":
                        saw_terminal = True
                    yield envelope
        ...
        # `messages_stream`'s job), just let the generator close.
        raise
...
async def messages_stream(
    ...
    """``POST /v1/messages`` with ``stream=True``. Async-generator function
    -- iterate with ``async for``. Completes normally only after observing
    the Anthropic wire terminal condition (``message_stop``); any other
    end-of-iteration raises an :class:`AgenticError` describing exactly
    why, per ADR-0024a §D5.
    """
