...
__export(meta_proxy_exports, {
  CLOUD_ROUTING_CONSENT_KIND: () => CLOUD_ROUTING_CONSENT_KIND,
  DEFAULT_META_PROXY_ORIGIN: () => DEFAULT_META_PROXY_ORIGIN,
  DEFAULT_META_PROXY_TOKEN_ENV_VAR: () => DEFAULT_META_PROXY_TOKEN_ENV_VAR,
  DEFAULT_PROXY_CONNECT_TIMEOUT_MS: () => DEFAULT_PROXY_CONNECT_TIMEOUT_MS,
  ...
  assertConsentForRoutingIntent: () => assertConsentForRoutingIntent,
  ...
  forwardChatCompletionStream: () => forwardChatCompletionStream,
  hasValidConsentGrant: () => hasValidConsentGrant,
  intentTouchesPlane: () => intentTouchesPlane,
  isBearerAttachmentAllowed: () => isBearerAttachmentAllowed,
  isBrowserLikeRuntime: () => isBrowserLikeRuntime,
  isConsentGrantValid: () => isConsentGrantValid,
  rejectRedirectResponse: () => rejectRedirectResponse,
  resolveMetaProxyClientConfig: () => resolveMetaProxyClientConfig,
  resolveProxyTimeBudget: () => resolveProxyTimeBudget
});
...
function fingerprintOf(token) {
  return (0, import_node_crypto3.createHash)("sha256").update(`${PRODUCT4}:${token}`).digest("hex").slice(0, 16);
}
