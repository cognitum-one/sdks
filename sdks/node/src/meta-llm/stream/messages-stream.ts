/**
 * `messagesCreateStream` HTTP + SSE orchestration (ADR-0024a §D5). Issue
 * #58 / M2 continuation, item 2 of the tracked "what's left" list —
 * Anthropic Messages streaming on top of the same generic
 * `../../sse/parser.js` PR #88 wired `chat.completionsStream` onto. The
 * pre-byte and post-byte harness below is intentionally the exact same
 * shape as `./chat-completions-stream.ts` — only the URL path, the
 * event-decode call, and the terminal condition differ; see that file's
 * module doc for the full retry/timeout/cancellation rationale, which
 ...
 * nonstream call" — streams are excluded — so no `Idempotency-Key` header
 * is sent here either.
 *
 ...
 * events were already `yield`ed standing as the partial result.
 *
 ...
 * the wire terminal condition") — NOT `[DONE]`/`finish_reason`, which are
 * OpenAI-specific.
 */
...
import { decodeAnthropicSseEvent, type AnthropicStreamEvent } from "./anthropic-events.js";
import type { MetaLlmStreamEnvelope } from "./envelope.js";

const PRODUCT = "meta-llm";
const OPERATION = "messages.createStream";

/**
 * `POST /v1/messages` with `stream: true`. Returns an async generator of
 * `MetaLlmStreamEnvelope<AnthropicStreamEvent>` — iterate with `for
 * await`. The generator completes normally only after observing the
 * Anthropic wire terminal condition (`message_stop`); any other
 * end-of-iteration throws an `AgenticError` describing exactly why, per
 * ADR-0024a §D5.
 */
export async function* messagesCreateStreamImpl(
  deps: NonstreamDeps,
...
function buildEnvelopes(
  ...
  requestId: string,
  nextSequence: () => number,
): MetaLlmStreamEnvelope<AnthropicStreamEvent>[] {
 ...
 * correctly-coded error. Identical to `./chat-completions-stream.ts`'s
