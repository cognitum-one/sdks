...
- **Date:** 2026-07-18
- **Updated:** 2026-07-19 — §D13 redaction sentinel (`SentinelSecretRedactor`: fixed-format matchers, charset-scoped Shannon-entropy fallback, bounded-depth-8 DFS, cycle detection) is implemented (PR #82). `ExecutionReceipt`/`LineageReference` construction and §D5-D9 verification (shape/digest/cryptographic/anchored ladder, HMAC-SHA256, lineage-chain structural validation) are implemented (PR #84), including a cross-language canonical-JSON golden fixture (`sdks/fixtures/receipt-canonicalization/`) added after independent review found a real camelCase-vs-snake_case cross-SDK signature-verification break. §D1's `TelemetrySink`/`TelemetryEvent` interface (with a functional `NoopTelemetrySink` default) and §D3's `cognitum.*` attribute name constants are now frozen as type-only scaffolding across all three languages (M6 first pass, tracking issue #70): no product client (`meta_llm`/`meta_proxy`/`metaharness`/`harnessaas`) emits through this interface yet, and §D2 trace propagation, §D4's event/metric catalog, and §D10 diagnostic capture remain open — the broader shared telemetry pipeline (structured operation/protocol/tier/cache/escalation attributes flowing through a real observability backend) is not yet built. Following the same pattern as this M6 first pass, §D2's trace-context parse/generate/join logic (real W3C `traceparent`/`tracestate` validation, generation, and join-vs-generate semantics against the frozen `TraceContext` carrier) and the four stable `cognitum.<product>.<operation>` span-name builder functions are now implemented as type-only-adjacent primitives across all three languages: no product client wires these into a real HTTP request yet, and OpenTelemetry adapter integration, baggage propagation, and the product allowlist for forwarded `tracestate` keys remain open. §D4's event/metric catalog and §D10 diagnostic capture are still unimplemented. Following the same pattern again, §D4's 17 telemetry event-name constants (`EVENT_REQUEST_START` through `EVENT_TELEMETRY_DROPPED`, one per literal dotted name in the ADR's catalog) and its 19-instrument metric catalog (`MetricInstrumentKind`/`MeasurementKind`, expanding each prose bullet — e.g. "input, output, cache, and safety token counters" — into one histogram-or-counter-classified instrument apiece) are now frozen as type-only scaffolding across all three languages, alongside `./telemetry.rs`/`.ts`/`.py` in a new sibling `telemetry_metrics.rs`/`telemetry-metrics.ts`/`telemetry_metrics.py` module: no emit call site exists, no OpenTelemetry meter or exporter is wired, and no product client (`meta_llm`/`meta_proxy`/`metaharness`/`harnessaas`) references these symbols yet. §D10 diagnostic capture remains the only unimplemented D-section from this group. Following the same pattern once more, §D10's `DiagnosticPolicy`/manifest-preview logic is now frozen across all three languages, alongside `./telemetry.rs`/`.ts`/`.py` and `./telemetry_metrics.rs`/`.ts`/`.py` in a new sibling `diagnostics.rs`/`.ts`/`.py` module: `DiagnosticPolicy` (included fields, max bytes/duration, `DiagnosticSink` local-path-or-callback, encryption/access-expectation, `RetentionPolicy`, and `allowed_categories` reusing the existing frozen `D12Category` taxonomy from `./sentinel.rs`/`.ts`/`.py` rather than a parallel enum — §D10's prompt/output/source/patch/tool/environment categories map onto 6 of `D12Category`'s 11 variants: `Prompts`/`Messages`/`Source`/`Patches`/`ToolArgumentsResults`/`EnvironmentValues`), `DiagnosticManifest` (`would_capture`/`blocked_by_policy`), and `DiagnosticBundle`/`RedactionReport` (type-only stub shapes) are all frozen and unit-tested. The hard-coded, policy-independent never-capturable set from §D10's prose ("credentials, signing private keys, proxy tokens, cookies, repository credentials, and pre-signed URLs are never capturable") resolves onto exactly 2 `D12Category` variants — `Credentials` (covering all of the credential/signing-key/proxy-token/cookie/repository-credential sub-list, since `D12Category` has no finer split and D13's own key-name rule already treats them uniformly) and `SignedUrls` — and `preview_diagnostic_manifest`'s real, tested logic enforces that this hard block always wins even when a caller's policy explicitly lists `Credentials`/`SignedUrls` in `allowed_categories`. No real capture/collection logic, no upload logic (per §D10, upload remains "a separate source-upload consent operation"), and no product client (`meta_llm`/`meta_proxy`/`metaharness`/`harnessaas`) references any symbol in this module yet. This closes out the M6 telemetry group opened by tracking issue #70 — §D1-§D4 and §D10 are now all frozen scaffolding across all three languages, with the real emission pipeline, OpenTelemetry adapter, and product wiring still open for a future pass.
