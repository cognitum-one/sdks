...
- **Updated:** 2026-08-05 — the `0.4.0-rc.1` npm staging rehearsal completed successfully and was rejected with registry-owner 2FA; no public prerelease remains. The `release` environment reviewer is the active release maintainer `proffesor-for-testing` (the previously configured reviewer was unavailable), with self-review explicitly enabled for this sole active maintainer. PyPI trusted-publisher proof remains the release gate; crates.io credential readiness must also be verified before an irreversible stable tag.
...
- npm trusted publishing is configured for both staging and publication, and the `0.4.0-rc.1` stage was inspected and rejected without public exposure;
- the `release` environment is protected by the active release maintainer `proffesor-for-testing`; self-review is enabled only because no second active reviewer is currently available;
- the coordinated Python and crates.io release is authorized, with the PyPI
  trusted-publisher binding and crates.io environment credential still
  requiring proof before the stable workflow;
