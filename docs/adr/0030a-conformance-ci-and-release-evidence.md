...
- **Date:** 2026-07-18
- **Updated:** 2026-08-05 — reconciled the ADR with the implemented repository. CI now includes cross-platform language gates, per-language coverage ratchets, a Rust feature matrix, capability-manifest validation, an aggregate GA gate, protected release tooling, and independent published-artifact live smoke for Node, Python, and Rust. The Domain and Wire fixture layers are implemented under `sdks/fixtures/`. Still open: Security-and-governance and Packaging fixture layers; the complete multi-runtime matrix; rendered cross-language product/compatibility reports; generated-code drift checks; mutation/regenerability evidence; and the full provenance/evidence bundle. Public-repository preparation is tracked in the private Cognitum coordination workspace.
