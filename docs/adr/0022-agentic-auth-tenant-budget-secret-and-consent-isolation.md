...
- **Date:** 2026-07-18
- **Updated:** 2026-07-19 — `CredentialProvider`/`RedactedSecret`/`SecretRedactor` frozen and built (PR #79 stub, PR #83 `StaticApiKeyCredentialProvider`, PR #82 `SentinelSecretRedactor`); D7 consent-grant isolation for Meta Proxy's `cognitum_cloud` routing plane (`ConsentGrant`/`ConsentRequiredError`) is implemented (PR #96). §D5 scope preflight and `OAuthTokenCredentialProvider` (Bearer-scheme, injectable refresh callback, no PKCE/auth-code flow) are implemented (PR #99), gating Meta LLM's completion-family routes without regressing the existing API-key path. Client-side `BudgetPolicy` guard exists as an additional (non-authoritative) check on Meta LLM operations; server-authoritative budget/reservation accounting is platform-side and out of SDK scope. Sponsor/power-saver/direct-Anthropic/training consent grants beyond the cloud-routing check are not yet implemented.
